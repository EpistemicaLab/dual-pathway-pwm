...
Per methods/method-J-translator-net.md: train a small translator MLP T_phi: z_A -> z_B
jointly with both encoders. Replace pointwise ||z_A - z_B||^2 with
||T_phi(z_A) - z_B||^2 + ||T_phi^inv(z_B) - z_A||^2 + alpha * cycle terms.
Cycle-consistency penalty defends against rank-deficient T collapse.

Loss: L_total = recon_a + recon_b + lambda_c * L_translator
   where L_translator = ||T_phi(z_proj_a) - z_proj_b||^2
                      + ||T_phi^inv(z_proj_b) - z_proj_a||^2
                      + alpha * (||T_phi^inv(T_phi(z_proj_a)) - z_proj_a||^2
                                 + ||T_phi(T_phi^inv(z_proj_b)) - z_proj_b||^2)

DEVIATION FROM SPEC (documented; mirrors AC/F/G/A/F0/C/B/I/H):
  Spec calls for joint encoder retraining. MVP is post-hoc on z_A_pre, z_B_pre
  (32-d section-5.7 baseline latents at lambda_C=1.0). Per-pathway projection MLPs
  (enc + dec) trained with reconstruction (acting as L_A, L_B 'predictive' terms)
  plus lambda_C * L_translator on z_proj_a, z_proj_b (16-d).

Output: results/method-J.json with PASS_GATE 4-gate evaluator
(c1, pass_linear, pass_cka, pass_transfer) per the original spec.

Honest-negative paths:
  * n_eval_paired < 100
  * ALL 2 sweep cells (depth in {1, 2}) diverge to NaN
...
def _translator_dims(z_dim: int, hidden: int, depth: int):
    """Translator dims: depth=1 is linear (z_dim -> z_dim); depth>=2 has hidden layers.

    depth=1: [z_dim, z_dim]
    depth=2: [z_dim, hidden, z_dim]
    depth=3: [z_dim, hidden, hidden, z_dim]
    """
    if depth == 1:
        return [z_dim, z_dim]
    return [z_dim] + [hidden] * (depth - 1) + [z_dim]
...
def _init_g(key, x_dim: int, z_dim: int, enc_h: int, dec_h: int,
            translator_h: int, depth: int):
    """Returns dict with enc_a, enc_b, dec_a, dec_b, T_phi, T_phi_inv MLPs."""
    ...
    t_dims = _translator_dims(z_dim, translator_h, depth)
    T_phi = _init_mlp(keys[4], t_dims)
    T_phi_inv = _init_mlp(keys[5], t_dims)
    return {
        "enc_a": enc_a, "enc_b": enc_b, "dec_a": dec_a, "dec_b": dec_b,
        "T_phi": T_phi, "T_phi_inv": T_phi_inv,
    }


# --- Translator-net cycle-consistent loss ---
def _translator_loss(z_a, z_b, T_phi, T_phi_inv, alpha):
    """Forward MSE + inverse MSE + alpha * symmetric cycle-consistency."""
    Tza = _mlp_forward(T_phi, z_a)            # T_phi(z_a) -> predicted z_b
    Tinvzb = _mlp_forward(T_phi_inv, z_b)     # T_phi_inv(z_b) -> predicted z_a
    fwd = jnp.mean(jnp.sum((Tza - z_b) ** 2, axis=-1))
    inv = jnp.mean(jnp.sum((Tinvzb - z_a) ** 2, axis=-1))
    # Cycle: T_phi_inv(T_phi(z_a)) ~ z_a; T_phi(T_phi_inv(z_b)) ~ z_b
    cyc_a = jnp.mean(jnp.sum((_mlp_forward(T_phi_inv, Tza) - z_a) ** 2, axis=-1))
    cyc_b = jnp.mean(jnp.sum((_mlp_forward(T_phi, Tinvzb) - z_b) ** 2, axis=-1))
    cycle = cyc_a + cyc_b
    total = fwd + inv + alpha * cycle
    return total, fwd, inv, cycle


def _g_step(params, x_a, x_b, lambda_c, alpha):
    """Joint loss: recon_a + recon_b + lambda_c * L_translator. MINIMIZED."""
    ...
    L_trans, fwd, inv, cycle = _translator_loss(
        z_a, z_b, params["T_phi"], params["T_phi_inv"], alpha
    )
    loss = recon_a + recon_b + lambda_c * L_trans
    aux = {
        "recon_a": recon_a,
        "recon_b": recon_b,
        "translator_total": L_trans,
        "translator_fwd": fwd,
        "translator_inv": inv,
        "translator_cycle": cycle,
...
def _train_one_seed(z_a_tr, z_b_tr, z_a_va, z_b_va, cfg, lambda_c: float,
                    depth: int, seed: int):
    """Returns (params, history, final). Mini-batch Adam over j_epochs."""
    z_dim = int(cfg["j_latent_dim"])
    enc_h = int(cfg["j_enc_hidden"])
    dec_h = int(cfg["j_dec_hidden"])
    translator_h = int(cfg["j_translator_hidden"])
    alpha_cycle = float(cfg["j_alpha_cycle"])
    bs_cfg = int(cfg["j_batch_size"])
    n_epochs = int(cfg["j_epochs"])
    lr = float(cfg["j_lr"])
    ...
    params = _init_g(
        ...
        translator_h=translator_h, depth=int(depth),
    )
    ...
    def step(params, opts, x_a, x_b):
        (loss, aux), g = grad_fn(params, x_a, x_b, lambda_c, alpha_cycle)
        new, new_opts = _adam_apply(params, opts, g, lr)
        return new, new_opts, loss, aux

    @jax.jit
    def eval_step(params, x_a, x_b):
        return _g_step(params, x_a, x_b, lambda_c, alpha_cycle)
    ...
    for ep in range(n_epochs):
        ...
        history.append({
            ...
            "val_translator_total": float(val_aux["translator_total"]),
            "val_translator_fwd": float(val_aux["translator_fwd"]),
            "val_translator_inv": float(val_aux["translator_inv"]),
            "val_translator_cycle": float(val_aux["translator_cycle"]),
...
def main():
    ...
    # 1) Build §5.7 baseline pair at lambda_C=1.0 (same as AC/F/G/A/F0/C/B/I/H).
    import run as d1_run  # type: ignore[import-not-found]
    z_a_pre, z_b_pre, meta = d1_run.build_baseline_pair(cfg)
    n = z_a_pre.shape[0]
    print(f"[J] baseline pair built. n={n} dimA={meta['dim_A']} dimB={meta['dim_B']} "
    ...
    # 3) 70/30 head split (same head_split_seed=271828 as AC/F/G/A/F0/C/B/I/H/D3/D4).
    ...
    # 4) Hyperparameter sweep over translator depth.
    depth_grid = list(cfg["j_depth_grid"])
    lambda_c_fixed = float(cfg["j_lambda_c_fixed"])
    sweep_seed = int(cfg["j_sweep_seed"])
    cell_records = []
    nan_cells = 0
    for depth in depth_grid:
        t_cell = time.perf_counter()
        params_cell, _hist, fin = _train_one_seed(
            z_a_tr, z_b_tr, z_a_te, z_b_te, cfg, lambda_c=lambda_c_fixed,
            depth=int(depth), seed=sweep_seed,
        )
        # Translator param count (T_phi + T_phi_inv).
        t_param_count = (_count_mlp_params(params_cell["T_phi"])
                         + _count_mlp_params(params_cell["T_phi_inv"]))
        rec = {"depth": int(depth), "lambda_c": lambda_c_fixed, **fin,
               "translator_param_count": int(t_param_count),
        ...
        print(f"[J] sweep cell depth={depth}: val_loss={rec.get('val_loss')!r} "
              f"params={t_param_count} wall={rec['wall_s']}s", flush=True)

    if nan_cells == len(depth_grid):
        result = {
            "verdict": "HONEST_NEGATIVE",
            "reason": "All translator-net sweep cells diverged to NaN -- optimization unstable on this pair.",
    ...
    print(f"[J] best cell depth={best_depth} val_loss={best_cell['val_loss']:.4f}", flush=True)

    # 5) 20-seed paired-bootstrap on best cell.
    seed_base = int(cfg["j_seed_base"])
    n_seeds = int(cfg["j_n_seeds"])
    seed_records = []
    best_seed_id = -1
    ...
    # 9) Cycle-consistency residual diagnostic on best seed (test slice).
    Tza_te = np.asarray(_mlp_forward(best_params["T_phi"], jnp.asarray(z_proj_a_te, dtype=jnp.float32)))
    cyc_te = np.asarray(_mlp_forward(best_params["T_phi_inv"], jnp.asarray(Tza_te, dtype=jnp.float32)))
    cycle_residual_te = float(np.mean(np.sum((cyc_te - z_proj_a_te) ** 2, axis=-1)))
    # Translator forward / inverse residuals on test (diagnostic only; PASS_GATE uses z_proj).
    Tinvzb_te = np.asarray(_mlp_forward(best_params["T_phi_inv"], jnp.asarray(z_proj_b_te, dtype=jnp.float32)))
    forward_residual_te = float(np.mean(np.sum((Tza_te - z_proj_b_te) ** 2, axis=-1)))
    inverse_residual_te = float(np.mean(np.sum((Tinvzb_te - z_proj_a_te) ** 2, axis=-1)))
    translator_param_count = (
        _count_mlp_params(best_params["T_phi"])
        + _count_mlp_params(best_params["T_phi_inv"])
    )
    ...
    headline = {
        ...
        "j_head": {
            "latent_dim": int(cfg["j_latent_dim"]),
            "enc_hidden": int(cfg["j_enc_hidden"]),
            "dec_hidden": int(cfg["j_dec_hidden"]),
            "translator_hidden": int(cfg["j_translator_hidden"]),
            "alpha_cycle": float(cfg["j_alpha_cycle"]),
            "lr": float(cfg["j_lr"]),
            "epochs": int(cfg["j_epochs"]),
            "batch_size_cfg": int(cfg["j_batch_size"]),
            "batch_size_effective": int(min(cfg["j_batch_size"], n_train)),
            "n_seeds": int(n_seeds),
            "best_cell": best_cell,
            ...
            "translator_depth": int(best_depth),
            "translator_param_count": int(translator_param_count),
            "cycle_consistency_residual": float(cycle_residual_te),
            "forward_residual_te": float(forward_residual_te),
            "inverse_residual_te": float(inverse_residual_te),
        },
        "best_cell_depth": int(best_depth),
        ...
        "pass_any": pass_any,
        "deviation_from_spec": (
            "POST-HOC translator-network projection on z_a_pre/z_b_pre (32-d section-5.7 baseline "
            "latents at lambda_C=1.0) instead of joint encoder retraining with translator T_phi "
            "replacing L_C. Per-pathway projection MLPs (enc + dec) trained with reconstruction "
            "(acting as the L_A, L_B 'predictive' terms in the spec) plus lambda_C * "
            "(forward_mse + inverse_mse + alpha * cycle). Translator T_phi: z_proj (16-d) -> "
            "z_proj (16-d), depth in {1, 2}; inverse T_phi_inv has same depth, separate weights. "
            "Mirrors AC/F/G/A/F0/C/B/I/H post-hoc choice. delta_1 / epsilon_2 not exposed by "
            "build_baseline_pair -> pass_linear forced FALSE; pass_cka and pass_transfer carry verdict."
        ),
