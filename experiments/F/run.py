...
def main():
    ...
    # CRITICAL: use the un-ablated u (rebuild via _build_aux_u) so F0's δ_1 measures
    # whether the encoder still recovers true u despite seeing zeroed u in training.
    ...
    # Constants from paper App. B.5 L356 (H=0 / g_ρ=1 anchor) — approximation transferred
    # from cond4/cond5 surrogate to F/F0 setting (cross-setup approximation; report raw budgets).
    APP_B5_Q = 0.02173
    APP_B5_SIGMA_OP = 5.008e-4
    APP_B5_M_R = 1.0  # bounded loss assumption A.1.5
    ...
    # PASS_LINEAR gate: K^* < M_R (Theorem A's informativeness criterion)
    ...
    # Protocol from VERIFY-3 (Attack #5 §5):
    #   σ_min(J_θ̂) median ≥ 10⁻² on c_other block, finite-difference on 10⁴ points.
    # We compute:
    #   (a) σ_min(J_full = ∂z_μ/∂z_pre) — full encoder Jacobian, 16×32
    #   (b) ∂z_μ/∂u — u-direction Jacobian, 16×4
    #   (c) c_other basis = orthogonal complement of column-space of (b), 12-d
    #   (d) J_restricted = c_other_basis^T @ J_full, 12×32
    #   (e) σ_min(J_restricted) median across 10⁴ points
    ...
    x_base = z_a_te_pre[jacob_idx]  # (n, 32)
    u_aud = u_te_true[jacob_idx]    # (n, 4)
    in_dim = int(x_base.shape[1])
    u_dim = int(u_aud.shape[1])
    eps_x = 1e-3
    eps_u = 1e-3

    # Build (n, 1+in_dim+u_dim, 32) and (n, 1+in_dim+u_dim, 4) batched perturbations
    # Index 0: base; indices 1..in_dim: perturb x dim i; indices in_dim+1..in_dim+u_dim: perturb u dim j
    n_p = 1 + in_dim + u_dim
    ...
    z_b = z_pert_all[:, 0, :]                    # (n, 16) base
    z_x = z_pert_all[:, 1:1+in_dim, :]            # (n, 32, 16) — perturb x
    z_u = z_pert_all[:, 1+in_dim:, :]             # (n, 4, 16) — perturb u

    J_full = (z_x - z_b[:, None, :]) / eps_x      # (n, 32, 16) → transpose to (n, 16, 32)
    J_full = J_full.transpose(0, 2, 1)
    J_u = (z_u - z_b[:, None, :]) / eps_u          # (n, 4, 16) → (n, 16, 4)
    J_u = J_u.transpose(0, 2, 1)

    # Full Jacobian σ_min
    svs_full = np.linalg.svd(J_full, compute_uv=False)  # (n, 16)
    ...
    # c_other: orthogonal complement of column space of J_u (per sample)
    sigma_min_other_list = []
    for i in range(n_jacob):
        U_u, S_u, _ = np.linalg.svd(J_u[i], full_matrices=True)  # U_u: (16, 16)
        c_other_basis = U_u[:, u_dim:]  # (16, 12)
        J_restricted = c_other_basis.T @ J_full[i]  # (12, 32)
    ...
    # V2 — TV-shifted OOD test via importance reweighting.
    # Hypothesis F.1-Gap: err_te(F0) - err_te(F) ≥ Ω(TV_cu) — gap grows with TV.
    # For 3-class uniform p_tr = (1/3, 1/3, 1/3), p_te = (a, (1-a)/2, (1-a)/2)
    # where a = 1/3 + tv_cu  (TV(p_te, p_tr) = a - 1/3 for a > 1/3).
    # Importance weight w_i = p_te(class_i) / p_tr(class_i) = 3 * p_te(class_i).
    n_classes_audit = 3
    ...
    result = {
        ...
        **meta,
        ...
        "pass_gate": {
            ...
            "tv_cells": tv_cells,
            "delta_1_note": (
                "TRACK A AUDIT 2026-06-06: δ_1 = max(OLS_A→u_true, OLS_B→u_true) on holdout, "
                "ε_2 = mean ||z_A^μ - z_B^μ||² on holdout, α_1 = σ_min of LSQ z_A→z_B (none-aligned), "
                "K* = α_1^{-1} √(1+‖Σ‖_op/q) (δ_1 + √ε_2) per ATTACK #3 cancellation. "
                "Constants ‖Σ‖_op=5.008e-4, q=0.02173, M_R=1.0 from paper App. B.5 L356 — "
                "cross-setup transfer from cond4/cond5 surrogate (caveat). "
                "F0 uses un-ablated u_true as regression target despite training on zeroed u."
            ),
            "c1_pass": c1_pass,
        ...
        "deviation_from_spec": (
            "POST-HOC iVAE on z_a_pre/z_b_pre (32-d §5.7 baseline latents at lambda_C=1.0) "
            "instead of raw x_A, x_B inputs. Khemakhem identifiability theory applies to any "
            "observation space; here the 'observations' are the L_C-aligned baseline latents. "
            "Trade-off: cheaper (no encoder retrain per seed), mirrors AC's post-hoc choice, "
            "and isolates F's marginal benefit on top of L_C-aligned base."
        ),
        "n_bootstrap": n_boot,
        "wall_s": round(time.perf_counter() - t0, 1),
        "git_head_at_run": _git_sha(HERE),
